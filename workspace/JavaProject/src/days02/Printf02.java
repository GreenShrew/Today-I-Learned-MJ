...
public class Printf02 {
	...
	public static void main(String[] args) {
		// printf 안에서 %와 d, f, s 사이에 숫자 옵션을 넣어서 표시할 자리수를 조절한다.

		// 정수의 자리수 조절
		// %와 d 사이에 숫자를 안 쓴 경우 : 표시될 데이터의 자리수만큼 확보한 후 출력.
		System.out.printf("정수%d정수\n", 123);
		
		// %와 d 사이에 숫자를 쓴 경우 : 쓴 숫자만큼 자리 확보후 출력데이터를 출력.
		// 만약 자리가 모자르다면 자동으로 자리를 추가한다. 자리가 남는다면 공백을 채워서 출력한다.
		System.out.printf("정수%3d정수\n", 12345);			// 공백을 3칸 만들어줬으나 들어오는 값은 5자리. 자리가 모자라다.
		System.out.printf("정수%10d정수\n", 12345);
		// 출력될 자리 확보 후 왼쪽 정렬.
		System.out.printf("정수%-10d정수\n", 12345);
		
		// 문자의 자리수 조절 - 정수와 크게 다르지 않다.
		...
		// 실수의 자리수 조절
		// 조절 숫자를 안 쓴 경우 : 소수점 윗 자리는 필요한 만큼 확보, 아래 자리는 6자리 확봐.
		System.out.printf("실수%f실수\n", 123.123);
		// 숫자를 쓴 경우
		// %10.2f : 총 10자리 확보 후 소수점 한자리 소수점 아래 숫자 2자리로 할당
		//				 따라서 소수점 윗 자리 숫자는 7자리가 할당된다.
		//				 소수점 윗 자리는 정수처럼 모자르면 추가 확보, 남으면 공백으로 채운다.
		// 소수점 아래는 표시될 자리만큼 표시하고 나머지를 자르거나, 남으면 0으로 채운다.
