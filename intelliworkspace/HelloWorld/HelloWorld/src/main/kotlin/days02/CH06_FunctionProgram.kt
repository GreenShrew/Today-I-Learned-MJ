...
fun main(){
    ...
    // 변수를 선언하고 초기화되는 값의 자료형이 명확하다면, 변수 선언의 자료형 부분은 생략이 가능하다.

    val r3 = testFun1(100,200)
    // 함수의 리턴값의 자료형이 Int 자료형으로 명확하기 때문에 따로 변수의 자료형을 쓰지 않아도 된다.
    val r4 = testFun2(100,200)
    val r5 = testFun3(100,200)
    println("-----------------------------------------")
    
    // 람다 함수 : 함수의 초간단 간략형 표현으로, 함수의 몸체를 변수에 담아 사용하도록 변형한 형태이다.
    val lambda1 : (Int, Int) -> Int = {a1:Int, a2:Int -> a1+a2} // 람다함수의 풀버전 사용
    // Nodejs의 익명함수 + 화살표함수와 비슷한 유형이다.
    // (Int, Int) : 매개변수의 갯수와 자료형
    // {a1:Int, a2:Int -> a1+a2} 앞에 정의된 자료형에 맞춰 사용되는 실제 매개변수와 연산식 & 리턴값
    // Int = { ,  -> a1+a2} : 리턴값의 자료형과 리턴값
    // lambda1 : 함수의 이름
    val r6 = lambda1(100, 200)  // 사용은 일반 함수와 같다.
    println("r6 : $r6")


    val lambda2 = {a1:Int, a2:Int-> a1+a2}  // 매개변수, 리턴값의 자료형이 명확하다면 ':' 이후부터 '=' 이전까지 생략가능하다.
    ...
    // 함수의 몸체에 여러라인의 명령이 필요한 경우 원래의 함수 형태 호출
...
// 3. 리턴값의 자료형이 아래와 같이 정수와 정수의 연산이어서 그 결과도 정수라면, 그리고 그 정수가 리턴될 것이라면
// 함수의 리턴 자료형도 생략가능하다.
fun testFun3(a1:Int, a2:Int) = a1 + a2


// 2. 함수의 리턴값이 간단한 수식으로 해결된다면 = 과 함께 연결하여 return 명령을 대신한다.
// testFun1과 TestFun2 함수는 같은 동작을 하는 함수이다.
